...
public class Node <N extends TreeData> {
    //left Tree
	private Node <N> left;
	//right Tree
     ...
     * @param node
     */
	public void insert(Node <N> node){
		//insert right
	...
	public Node <N> delete(TreeData data) {
		...
		//walk the tree recursivly
		 if(comp > 0){
			...
			if(left != null){
				...
				Node <N> iAMRoot = getNewRootNode(left);
	 ...
	 * @return
	 */
	private Node <N> getNewRootNode(Node <N> root) {
		if(root.isLeaf()){
			return root;
		} else {
			Node <N> iAmRoot;
