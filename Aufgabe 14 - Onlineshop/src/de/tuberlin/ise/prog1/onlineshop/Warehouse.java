...
public class Warehouse {
	...
	// Array wird zur Liste geändert

	private List<WarehouseItem> warehouseContent = new ArrayList<WarehouseItem>();
	...
	public void registerProducts(Product... products) {
		...
		// Liste vom Typ WarehouseItem wird erstellt
		List<WarehouseItem> newwarehousecontent = new ArrayList<WarehouseItem>();
		
		// alle daten der liste warehouseContent werden hinzugefügt
		newwarehousecontent.addAll(warehouseContent);
		
		// alle neuen produkte werden zu der neuen Liste hinzugefügt
		for (int i = 0; i < products.length; i++) {
			newwarehousecontent.add(new WarehouseItem(products[i]));

		}
		
		// die alte liste wird mit den inhalten der neuen Liste überschrieben
		warehouseContent.addAll(newwarehousecontent);
	}
	...
	/**
	 * Returns an instance of WarehouseItem where the associated product has the
	 * specified Id
	 * 
	 * @param id,
	 *            stock
	...
	/**
	 * Prints an inventory list in the form [product id, product, stock,
	 * shippingOption]
	 */
	public void printInventory() {
		for (WarehouseItem w : warehouseContent) {
			System.out.println("id: " + w.getProduct().getId() + ", product: " + w.getProduct().toString() + ", stock:"
					+ w.getStock());
		}
	}
